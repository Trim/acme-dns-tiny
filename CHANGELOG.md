<!--
SPDX-FileCopyrightText: 2026 Adrien Dorsaz
...
- error management on response of ACME server has been improved to read
  correctly response with errors (a warning message will be raised by 
  acme-dns-tiny by default)
...
This new major version will _require you update your configuration file_ to work.
...
`DNS` nameserver to send `DNS` resource record requests _AND_ to install the
...
resource records for _each domain_ defined on the certificate signing request
(the `CSR` file).
